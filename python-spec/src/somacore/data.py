...
class DataFrame(base.SOMAObject, metaclass=abc.ABCMeta):
    ...
    def create(
        ...
        *,
        ...
        ``soma_joinid`` of type ``pyarrow.int64``, with negative values disallowed.
        If a ``soma_joinid`` column is present in the provided schema, it must be of
        the correct type.  If no ``soma_joinid`` column is provided, one will be
        added. This column may be used as an indexed column.
